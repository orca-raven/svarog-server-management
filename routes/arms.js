...
module.exports = (db) => {
    ...
    // Получить все АРМ
    router.get('/', (req, res) => {
        const sql = `SELECT * FROM arms ORDER BY netbios_name COLLATE NOCASE`;
    ...
    router.get('/:id', (req, res) => {
        const sql = `SELECT * FROM arms WHERE id = ?`;
        ...
        db.get(sql, [req.params.id], (err, row) => {
            ...
            if (!row) {
                res.status(404).json({ error: 'АРМ не найден' });
                return;
            }
            res.json(row);
        });
    ...
    router.post('/', (req, res) => {
        ...
        const params = [
            netbios_name, domain || 'НЕТ', type || 'АРМ', inventory_number,
        ...
        db.run(sql, params, function(err) {
            ...
            res.json({ id: this.lastID, message: 'АРМ создан успешно' });
        });
    });
    ...
    router.put('/:id', (req, res) => {
        ...
        db.run(sql, params, function(err) {
            ...
            res.json({ message: 'АРМ обновлен успешно' });
        });
    });

    // Удалить АРМ
    router.delete('/:id', (req, res) => {
        const sql = `DELETE FROM arms WHERE id = ?`;
        ...
        db.run(sql, [req.params.id], function(err) {
            ...
            res.json({ message: 'АРМ удален успешно' });
        });
    });

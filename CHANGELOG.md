...
### Откат изменений
- Откат настроек API безопасности (версия 1.0.5)
- API снова работает по умолчанию без ограничений
- Удалены настройки включения/выключения API из интерфейса

### Причина отката
- API необходим для корректной работы фронтенда системы
- Отключение API нарушало базовую функциональность
- Система полностью зависит от API для взаимодействия интерфейса с backend

### Исправлено  
- Восстановлена полная функциональность системы
- API работает стабильно и доступен всегда
